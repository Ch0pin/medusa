            ...
            if download:
                try:
                    ...
                    if os.path.exists("./busybox.tmp"):
                        print(
                            "[i] Download successfull, pushing the binary to the device as '/data/local/tmp/{}'".format(
                                binary))
                        print(self.run_command(["adb", "-s", "{}".format(self.device.id), "push", "./busybox.tmp",
                                                "/data/local/tmp/{}".format(binary)]).decode('utf-8'))
                        print("[i] Deleting local file...")
                        print(self.run_command(["rm", "./busybox.tmp"]).decode('utf-8'))
                        print(self.run_command(["adb", "-s", "{}".format(self.device.id), "shell", "chmod", "+x",
                                                "/data/local/tmp/{}".format(binary)]).decode('utf-8'))
                        ...
                        with open(shellfile, 'w') as sf:
                            sf.write(data)
                        subprocess.run(
                            """adb -s {} push {} /data/local/tmp/busybox.sh""".format(self.device.id, shellfile),
                            shell=True)
                    else:
        ...
        if self.current_app_sha256 == None:
            ...
            try:
                if len(line.split()) > 1 and '--poc' in line.split()[1]:
                    ...
                    output = os.popen(
                        "adb -s {} shell am start -W -a android.intent.action.VIEW -d {}".format(self.device.id,
                                                                                                 line.split(' ')[
                                                                                                     0])).read()
                    print(output)
            except Exception as e:
    ...
    def do_installBurpCert(self, line):
        ...
        try:
            ...
            if a == 'y':
                ...
                os.popen(
                    "adb -s {} shell am broadcast -a com.medusa.INSTALL_CERTIFICATE -n com.medusa.agent/.Receiver".format(
                        self.device.id)).read()
...
#reboot
    ...
    def do_load(self, line):
        ...
        Load an application which already exists in the current (working) database."""
    ...
    def do_notify(self, line):
        ...
        try:
            ...
            if 'com.medusa.agent' in self.packages:
                output = os.popen(
                    "adb -s {} shell am broadcast  -a com.medusa.NOTIFY --es subject {} --es body {}".format(
                        self.device.id, line.split(' ')[0], line.split(' ')[1])).read()
                print(output)
            else:
                print(
        ...
        if os.path.exists(file):
            try:
                ...
                if 'android:debuggable="true"' in data:
                    ...
                    subprocess.run('java -jar ' + APKTOOL + ' b {} -o {}'.format(TMP_FOLDER, DEBUGGABLE_APK),
                                   shell=True)
                    ...
                    subprocess.run(
                        'apksigner sign --ks {} -ks-key-alias common --ks-pass pass:password --key-pass pass:password  {}'.format(
                            SIGNATURE, ALLIGNED_APK), shell=True)
    ...
    def do_playstore(self, line):
        ...
        print(os.popen(
            "adb -s {} shell am start -W -a android.intent.action.VIEW -d market://details?id={}".format(self.device.id,
                                                                                                         line.split(
                                                                                                             ' ')[
                                                                                                             0])).read())
    ...
    def do_pull(self, line):
        ...
        try:
            base_apk = os.popen(
                "adb -s {} shell pm path {} | grep base.apk | cut -d ':' -f 2".format(self.device.id, package)).read()
    ...
    def do_spawn(self, package):
        ...
        try:
            ...
            os.popen("adb -s {} shell  monkey -p {} -c 'android.intent.category.LAUNCHER 1'".format(self.device.id,
                                                                                                    package.split(' ')[
                                                                                                        0])).read()
    ...
    def do_start(self, line):
        ...
        if self.current_app_sha256 == None:
            ...
            try:
                cmd = "adb -s {} shell 'su -c \"am start -n {}/{}\"'".format(self.device.id, self.info[0][2],
                                                                             line.split(' ')[0])
    ...
    def do_startsrv(self, line):
        ...
        if self.current_app_sha256 == None:
            ...
            cmd = "adb -s {} shell 'echo \"am startservice -n {}/{}\" | su'".format(self.device.id, self.info[0][2],
                                                                                    line.split(' ')[0])
            print("adb command: {}".format(cmd))
            try:
                output = os.popen(cmd).read()
    ...
    def do_stopsrv(self, line):
        ...
        if self.current_app_sha256 == None:
            ...
            try:
                output = os.popen(
                    "adb -s {} shell 'echo \"am stopservice -n {}/{}\" | su'".format(self.device.id, self.info[0][2],
                                                                                     line.split(' ')[0])).read()
                print(output)
            except Exception as e:
                print(e)
        ...
        if self.current_app_sha256 == None:
            ...
            try:
                ...
                if not 'Error' in script:
                    if 'Darwin' in opsys:
                        subprocess.run(
                            """osascript -e 'tell application "Terminal" to do script "{}" ' """.format(script),
                            shell=True)
                    elif 'Linux' in opsys:
...
[------------------------------------------------------------------------------------------]
    ...
    def print_deeplinks(self, quite=False):
        ...
        for attribs in self.deeplinks:
            if component != attribs[0]:
                ...
                l = len(component)
                if not quite:
                    print(Fore.GREEN + '-' * l + Fore.YELLOW + '\nDeeplinks that start:' + Fore.CYAN + '{}'.format(
    ...
    def print_proxy(self):
        ...
        output = subprocess.run("""adb -s {} shell 'echo "iptables -t nat -L" | su'""".format(self.device.id),
                                shell=True)
        print(output)
    ...
    def init_application_info(self, application_database, app_sha256):
        try:
            ...
            self.activity_names = list('\n'.join(map(lambda x: str(x[0]), application_database.query_db(
                "SELECT name from Activities WHERE app_sha256='{}'".format(app_sha256)))).split('\n'))
            self.service_names = list('\n'.join(map(lambda x: str(x[0]), application_database.query_db(
                "SELECT name from Services WHERE app_sha256='{}'".format(app_sha256)))).split('\n'))
            self.manifest = application_database.query_db(
                "SELECT androidManifest FROM Application WHERE sha256='{}';".format(app_sha256))
            self.strings = application_database.query_db(
                "SELECT stringResources FROM Application WHERE sha256='{}';".format(app_sha256))[0][0].decode('utf-8')
    ...
    def transproxy(self, ip, port):
        ...
        try:
            ...
            os.popen(
                "adb -s {} shell 'chmod +x /data/local/tmp/transproxy.sh; echo \"/data/local/tmp/transproxy.sh {} {}\" | su; rm /data/local/tmp/transproxy.sh'".format(
                    self.device.id, ip, port)).read()
            self.print_proxy()
        except Exception as e:
            print(e)
